...
class trading(commands.Cog):
    ...
    async def trading_slash(self, ctx, trading_partner : Option(discord.Member, required=True), what_you_give: Option(str, choices=['Part(s)', 'Money'], required=True), what_you_want: Option(str, choices=['Part(s)', 'Money'], required=True)):
        await new_trading_member(ctx.author)
        await new_trading_member(trading_partner)
            ...
            if str(ctx.author.id) in bank_account:  
                ...
                if what_you_want == "Money":
                    if str(trading_partner.id) in bank_account: 
                        ...
                        #trading_list["trades"] = {}
                        trading_list["trades"][new_trading_count] = {}
                        trading_list["trades"][new_trading_count]["give_type"] = "coins"
                        trading_list["trades"][new_trading_count]["give_coins_amount"] = ans1
                        trading_list["trades"][new_trading_count]["want_type"] = "coins"
                        trading_list["trades"][new_trading_count]["want_coins_amount"] = ans2
                        ...
                        with open("trading_member.json", "w") as f:
                            json.dump(trading_member, f, indent=4)
                        return
                    else:
